...
export default function EducationView() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-neutral-100/90 dark:bg-neutral-900/90 backdrop-blur-md px-4 pt-8 pb-3">
      ...
      <div className="flex-1 overflow-auto px-4 pb-32 lg:pb-8 pt-4 grid grid-cols-1 lg:grid-cols-2 gap-2 content-start">
        {CATEGORIES.map((cat) => (
          <CategoryCard
            key={cat.id}
            category={cat}
...
function CategoryCard({ category, onClick }: { category: Category; onClick: () => void }) {
  ...
  return (
    <button
      ...
      className={`relative w-full text-left rounded-2xl overflow-hidden p-4 h-36 border border-black/20 dark:border-white/15 ${category.bg} transition-all hover:opacity-90`}
    >
