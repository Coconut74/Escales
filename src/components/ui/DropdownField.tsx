...
export default function DropdownField({ label, options, error, className = '', id, ...props }: DropdownFieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      {label && (
        <label htmlFor={selectId} className="text-base font-semibold text-neutral-700 dark:text-neutral-200">
          {label}
        </label>
      )}
      <div className="relative w-full">
        <select
          id={selectId}
          className={`
            w-full appearance-none px-4 py-3.5 pr-14 rounded-xl border text-base font-sans
        ...
        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none">
          <Icon name="arrow" size={20} className="rotate-90" />
        </span>
      </div>
