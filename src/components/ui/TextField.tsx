...
export default function TextField({ label, error, className = '', id, ...props }: TextFieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      {label && (
        <label htmlFor={inputId} className="text-base font-semibold text-neutral-700 dark:text-neutral-200">
          {label}
        </label>
      )}
      <input
        id={inputId}
        className={`
          w-full px-4 py-3.5 rounded-xl border text-base font-sans
